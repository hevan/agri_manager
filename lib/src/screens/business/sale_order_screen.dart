...
class SaleOrderScreen extends StatefulWidget {
  ...
  const SaleOrderScreen({Key? key, this.batchId, this.batchName})
      : super(key: key);
  ...
  Future loadData() async {
    ...
    try {
      var retData = await DioUtils().request(
          HttpApi.sale_order_query, "GET",
          queryParameters: params);
      if (retData != null) {
        setState(() {
          listData =
              (retData['content'] as List).map((e) => SaleOrder.fromJson(e)).toList();
        });
      }
