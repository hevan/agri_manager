...
import 'package:agri_manager/src/screens/business/purchase_order_item_edit_screen.dart';
...
class PurchaseOrderEditScreen extends StatefulWidget {
  ...
  const PurchaseOrderEditScreen({Key? key, this.id,  this.batchId})
      : super(key: key);
...
class _PurchaseOrderEditScreenState extends State<PurchaseOrderEditScreen> {
  ...
  List<PurchaseOrderItem> listOrderItem = [];
      ...
      try {
        ...
        if (retData != null) {
          setState(() {
            ...
            _textBatchName.text = _purchaseOrder.batchProduct!.name!;
            _textName.text = _purchaseOrder.name!;
            _textDescription.text = _purchaseOrder.description ?? '';
            _textOccurAt.text = _purchaseOrder.occurAt ?? '';
            _textCustomerName.text = _purchaseOrder.customer!.name!;
  ...
  Future save() async {
    ...
    try {
      _purchaseOrder.name = _textName.text;
      _purchaseOrder.description = _textDescription.text;
      _purchaseOrder.occurAt = _textOccurAt.text;
      _purchaseOrder.corpId = curCorp?.id;
      _purchaseOrder.createdUserId = userInfo.userId;
      _purchaseOrder.status = 0;
      _purchaseOrder.checkStatus = 0;

      var retData = await DioUtils().request(
          HttpApi.purchase_order_add, "POST",
          data: json.encode(_purchaseOrder), isJson: true);
  ...
  toDelateItem(PurchaseOrderItem item, int index){
    if(null != item.id){
  ...
  Future deleteOrderItem(int id) async {
    try {
      await DioUtils().request('${HttpApi.purchase_order_item_delete}${id}', "DELETE");
    } on DioError catch (error) {
      CustomAppException customAppException = CustomAppException.create(error);
      debugPrint(customAppException.getMessage());
      Fluttertoast.showToast(
   ...
   Future toAddExpenseItem() async{
     String retEditItem = await Navigator.push(
         context,
         MaterialPageRoute(builder: (context) =>  PurchaseOrderItemEditScreen(orderId: widget.id,),
             fullscreenDialog: true)
     );
     if(retEditItem != null) {
       // ignore: curly_braces_in_flow_control_structures
       setState(() {
         PurchaseOrderItem curItem  = PurchaseOrderItem.fromJson(json.decode(retEditItem));
         setState(() {
                      ...
                      onPressed: () async {
                        ...
                        if(retData != null) {
                          ...
                          setState(() {
                            ...
                            _purchaseOrder.batchId = retDataMap['id'];
                    ...
                    suffixIcon: IconButton(
                      onPressed: () async {
                        ...
                        if(retData != null) {
                          ...
                          setState(() {
                            var retDataMap = json.decode(retData);
                            _purchaseOrder.customerId = retDataMap['id'];
                    ...
                    suffixIcon: IconButton(
                      onPressed: () async {
                        final selected = await showDatePicker(
                          context: context,
                          initialDate: _purchaseOrder.occurAt != null
                              ? DateFormat('yyyy-MM-dd')
                                  .parse(_purchaseOrder.occurAt!)
                              : DateTime.now(),
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: TabBarView(children: [
              ...
              Container(
                child: null != _purchaseOrder.id ? DocumentScreen(entityId: _purchaseOrder.id!,entityName: 'purchaseOrder',groupName: '凭证',) : Center(),
              ),
