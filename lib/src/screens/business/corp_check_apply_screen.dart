...
class CorpCheckApplyScreen extends StatefulWidget {
  final bool isShowBar;
  const CorpCheckApplyScreen({Key? key, this.isShowBar = true}) : super(key: key);

  @override
  State<CorpCheckApplyScreen> createState() => _CorpCheckApplyScreenState();
}

class _CorpCheckApplyScreenState extends State<CorpCheckApplyScreen>{

  List<CheckApply> listData = [];
  ...
  Future loadData() async{
    var params = {'corpId': curCorp?.id,'userId': userInfo.userId, 'page': pageModel.page, 'size':pageModel.size};
    ...
    try {
      ...
      if(retData != null && null != retData['content']) {
        setState(() {
          listData = (retData['content'] as List).map((e) => CheckApply.fromJson(e)).toList();
        });
      }
                                ...
                                rows: List<DataRow>.generate(
                                    listData.length,
                                      ...
                                      DataCell(
                                          ...
                                          DataCell(
                                              Text(AgriUtil.showCheckStatus(listData[index].status))),
                                      DataCell(
                                          Row(children: [
                                            ElevatedButton(
                                              style:  elevateButtonStyle,
                                              onPressed: (){
                                                AgriUtil.toCheckView(context, entityId: listData[index].entityId!, entityName: listData[index].entityName!);
                                              },
                                              child: const Text('查看'),
                                            ),
                                          ],))
