...
class _CorpViewScreenState extends State<CorpViewScreen> {
  ...
  toEdit() {
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => CorpEditScreen(id: widget.data.id)),
    );
  }
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          ShowFieldText(
              title: '所在地区',
              data:
                  '${widget.data.address?.province ?? ''}${widget.data.address?.city ?? ''}${widget.data.address?.region ?? ''}'),
          const SizedBox(
            height: kSpacing,
          ),
          ShowFieldText(
              title: '详细地址', data: '${widget.data.address?.lineDetail ?? ''}'),
          const SizedBox(
            height: kSpacing,
          ),
          ShowFieldText(
              title: '联系人', data: '${widget.data.address?.linkName ?? ''}'),
          const SizedBox(
            height: kSpacing,
          ),
          ShowFieldText(
              title: '联系电话', data: '${widget.data.address?.linkMobile ?? ''}'),
          const SizedBox(
            height: kSpacing,
          ),
          ElevatedButton(
